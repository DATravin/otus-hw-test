...
resource "yandex_dataproc_cluster" "dataproc_cluster" {
  ...
  bucket      = var.yc_data_bucket #yandex_storage_bucket.data_bucket.bucket
  description = "Dataproc Cluster created by Terraform for OTUS project"
  name        = var.yc_dataproc_cluster_name
  labels = {
    created_by = "terraform"
  }
  service_account_id = var.service_acc_id#  yandex_iam_service_account.sa.id
  zone_id            = var.yc_zone
  security_group_ids = [var.security_group_id] # [yandex_vpc_security_group.security_group.id]
  ...
  cluster_config {
    ...
    subcluster_spec {
      ...
      subnet_id        = var.vpc_subnet_id #yandex_vpc_subnet.subnet.id
      hosts_count      = 1
